...
def main(filename):
    ...
    df = _remove_new_lines_column(df, 'body')
    df = _remove_new_lines_column(df, 'title')
...
def _extract_newspaper_uid(filename):
    ...
    logging.info('Newspaper uid detected: {}'.format(newspaper_uid))
...
def _remove_new_lines_column(df, column_name):
    logger.info('Remove new lines from {}'.format(column_name))
    stripped_body = df.apply(lambda row: row[column_name].replace('\n', ''), axis=1)
...
if __name__ == '__main__':
    ...
    df.drop(df[df['title'] == 'Opinión '].index, inplace=True)
    print(df)
